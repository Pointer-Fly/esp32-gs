...
void _ui_arc_set_text_value(lv_obj_t * trg, lv_obj_t * src, const char * prefix, const char * postfix)
{
    ...
    // if(trg==ui_qiyaLabel)lv_snprintf(buf, sizeof(buf), "%s%.3f%s", prefix, lv_arc_get_value(src)/1000.0, postfix);
    // else if(trg==ui_tempLabel)lv_snprintf(buf, sizeof(buf), "%s%.1f%s", prefix, lv_arc_get_value(src)/10.0, postfix);
    // else if(trg==ui_heightLabel)lv_snprintf(buf, sizeof(buf), "%s%.2f%s", prefix, lv_arc_get_value(src)/100.0, postfix);
    // else lv_snprintf(buf, sizeof(buf), "%s%d%s", prefix, (int)lv_arc_get_value(src), postfix);
